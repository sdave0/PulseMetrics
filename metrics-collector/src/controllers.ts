...
export class MetricsController {
  ...
  async getJobBreakdown(req: Request, res: Response) {
    try {
      const runId = req.query.run_id ? parseInt(req.query.run_id as string, 10) : undefined;
      const data = await metricsService.getJobBreakdown(req.query.pipeline as string, runId);
      res.status(200).json(data);
    } catch (error: unknown) {
  ...
  async triggerAnalysis(req: Request, res: Response) {
    ...
    try {
      // 1. Ensure signals exist (On-demand extraction for legacy runs)
      await metricsService.ensureSignals(runId);

      // 2. Fetch existing signals
      const bundle = await metricsService.getSignals(runId);

      // 3. Run Agentic Analysis
      const analysis = await aiService.analyzeFailure(bundle as any);
      ...
      if (analysis) {
        ...
        res.status(500).json({ error: "Agent failed to generate analysis." });
      }
  ...
  async getSignals(req: Request, res: Response) {
    ...
    try {
      const signals = await metricsService.getSignals(runId);
      res.status(200).json(signals);
    } catch (error: unknown) {
      const msg = `Error fetching signals: ${error instanceof Error ? error.message : String(error)}`;
      console.error(msg);
      res.status(500).send({ error: msg });
    }
  }

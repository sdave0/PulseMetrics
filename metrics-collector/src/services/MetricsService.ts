...
export class MetricsService {
  ...
  async processMetrics(payload: MetricsPayload): Promise<void> {
    ...
    try {
      // Fetch historical runs for baseline comparison
      const historicalRuns = await repo.getJobBreakdown(ANOMALY_WINDOW_SIZE, workflow.name);

      // Extract signals
      const signalBundle = await signalOrchestrator.extractSignals(payload, historicalRuns);

      // Persist all signals
      const allSignals: Signal[] = [
        ...signalBundle.patterns,
        ...signalBundle.drift,
        ...signalBundle.changes,
        ...signalBundle.recurrences,
      ];

      if (signalBundle.decision) {
        allSignals.push({
          type: 'decision',
          name: 'decision_engine',
          confidence: 1.0,
          timestamp: new Date(),
          ...signalBundle.decision
        } as unknown as Signal);
      }

      if (signalBundle.aiAnalysis) {
        const { confidence, ...analysisData } = signalBundle.aiAnalysis;
        allSignals.push({
          type: 'ai_analysis',
          name: 'agent_rca',
      ...
      console.log(`Extracted and persisted ${allSignals.length} signals for run ${workflow.run_id}`);
    } catch (error) {
      console.error(`Error extracting signals for run ${workflow.run_id}:`, error);
      // Don't fail the entire metrics processing if signal extraction fails
    }
  }

  async getSignals(runId: number) {
    const signals = await repo.getSignalsByRun(runId);

    // Group signals by type
    const patterns = signals.filter(s => s.signal_type === 'pattern').map(s => s.value);
    const drift = signals.filter(s => s.signal_type === 'drift').map(s => s.value);
    const changes = signals.filter(s => s.signal_type === 'change').map(s => s.value);
    const decisionSignal = signals.find(s => s.signal_type === 'decision');
    const aiAnalysisSignal = signals.find(s => s.signal_type === 'ai_analysis');
    ...
    return {
      ...
      drift,
      changes,
      decision: decisionSignal ? decisionSignal.value : undefined,
      aiAnalysis: aiAnalysisSignal ? aiAnalysisSignal.value : undefined,
      generatedAt: signals[0]?.created_at || new Date(),
    };
  }

  async ensureSignals(runId: number): Promise<void> {
    const existing = await repo.getSignalsByRun(runId);
    // If we have any signals (pattern, drift, change, decision), we assume it's processed.
    if (existing.length > 0) return;

    console.log(`[PulseAI] No signals found for run ${runId}. Performing on-demand extraction...`);

    // 1. Fetch Run Context
    const rows = await repo.getRunContext(runId);
    if (rows.length === 0) {
      throw new Error(`Run ${runId} not found in database.`);
    }

    const targetRun = rows[0];
    ...
    const payload: MetricsPayload = {
      workflow: {
        ...
        name: targetRun.workflow_name || 'unknown',
        status: targetRun.status || null,
        duration_seconds: targetRun.duration_seconds || 0,
        created_at: targetRun.created_at.toISOString(),
        html_url: targetRun.html_url || '',
        run_number: targetRun.run_number,
        trigger: targetRun.trigger_event || 'unknown', // Map trigger_event to trigger
        branch: targetRun.branch || '',
        completed_at: null // Not strictly needed for extraction usually
      },
      commit: {
        sha: targetRun.commit_sha || '',
        message: targetRun.commit_message || '',
        author: targetRun.commit_author || '',
        parent_sha: targetRun.commit_parent_sha || '',
        // timestamp removed as it is not in Commit interface
      },
      jobs: (targetRun.jobs || []) as Job[],
      test_summary: targetRun.test_summary || null,
      commit_analysis: targetRun.commit_analysis || null,
      build_analysis: targetRun.build_analysis || null,
      artifacts: []
    };
    ...
    // 4. Persist Signals
    // Reuse the logic from processMetrics or just manual save
    const allSignals: Signal[] = [
      ...signalBundle.patterns,
      ...signalBundle.drift,
      ...signalBundle.changes,
  ...
  async saveAiAnalysis(runId: number, analysis: any) {
    // 1. Check if an AI signal already exists for this run to avoid duplicates or update it
    // For simplicity, we'll just insert a new one or let repo method handle it.
    ...
    await repo.saveSignal(
      ...
      analysis,
      1.0 // High confidence since it's user-invoked analysis
    );
  }
  ...
  async getDurationAnalysis(pipeline?: string) {
    ...
    return runs.map((run, index, allRuns) => {
      ...
      const sum = window.reduce((acc: number, cur: RunChartData) => acc + Math.max(0, cur.duration_seconds ?? 0), 0);
      cumulativeAvg = sum / (index + 1);

      if (index >= ANOMALY_WINDOW_SIZE) {
        const anomalyWindow = allRuns.slice(index - ANOMALY_WINDOW_SIZE, index);
        const anomalySum = anomalyWindow.reduce((acc: number, cur: RunChartData) => acc + (cur.duration_seconds || 0), 0);
